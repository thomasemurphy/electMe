...
state_df = pd.read_csv("nomination_state_table.csv")
...
for current_week in range(1,24):                            #24 election weeks in the game
    ...
    if(len(state_df[state_df.ELECTION_WEEK == current_week])) > 0:
        ...
        for i in range(0,len(player_df.index)):
            ...
            new_player_pop = pd.Series(state_df[state_df.ELECTION_WEEK == current_week][player] + \
                    delta_econ.apply(polling) + delta_soc.apply(polling), name = player) #population to add based on polling function of those deltas
